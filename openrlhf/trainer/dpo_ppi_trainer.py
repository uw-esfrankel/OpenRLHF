        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for data in self.train_dataloader:
                if not self.packing_samples:
                    chosen_ids, c_mask, reject_ids, r_mask, pseudo_label_agreement, gold_label_agreement, prompt_id_lens = data
                    ...
                    packed_input_ids, packed_attention_masks, packed_seq_lens, pseudo_label_agreement, gold_label_agreement, prompt_id_lens = data
        ...
        with torch.no_grad():
            ...
            for data in eval_dataloader:
                if not self.packing_samples:
                    ...
                    chosen_ids, c_mask, reject_ids, r_mask, _, _, prompt_id_lens = data
                    chosen_ids = chosen_ids.squeeze(1).to(torch.cuda.current_device())
                    c_mask = c_mask.squeeze(1).to(torch.cuda.current_device())
                    reject_ids = reject_ids.squeeze(1).to(torch.cuda.current_device())
                    r_mask = r_mask.squeeze(1).to(torch.cuda.current_device())
                    ...
                    packed_input_ids, packed_attention_masks, packed_seq_lens, _, _, prompt_id_lens = data
                    packed_input_ids, packed_attention_masks = packed_input_ids.to(
                        torch.cuda.current_device()
                    ), packed_attention_masks.to(torch.cuda.current_device())
